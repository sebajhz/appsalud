...
- **Cabecera E5.10.2 (Liquidity Sweep Quality V1 — observación, sin gate):** inmediatamente después de `liquidity_event_reasons`, antes de `session_bucket`:  
  `liquidity_sweep_quality_score,liquidity_sweep_quality_grade,liquidity_sweep_recency_score,liquidity_sweep_directional_score,liquidity_sweep_reaction_score,liquidity_sweep_displacement_score,liquidity_sweep_distance_score,liquidity_sweep_quality_reasons`  
- **Cabecera E5.10.4 (Causal Liquidity Chain V1 — observación):** inmediatamente después de `liquidity_sweep_quality_reasons`, antes de `session_bucket`:  
  `liquidity_chain_detected,liquidity_chain_grade,liquidity_chain_score,liquidity_chain_sweep_to_setup_bars,liquidity_chain_sweep_to_fvg_bars,liquidity_chain_reaction_confirmed,liquidity_chain_displacement_confirmed,liquidity_chain_fvg_created_after_sweep,liquidity_chain_distance_to_fvg_points,liquidity_chain_reasons`  
- **Cabecera E5.10.6 (Liquidity Chain Reaction Audit — observación):** inmediatamente después de `liquidity_chain_reasons`, antes de `session_bucket`:  
  `liquidity_chain_reaction_failure_reason,liquidity_chain_reaction_close_price,liquidity_chain_reaction_level,liquidity_chain_reaction_bars_checked`  
  Cuando **`has_liquidity_chain_reaction_audit_v1_logic`** es **true** en `backtest_summary.json`, el EA exporta también los contadores agregados `liquidity_chain_reaction_*` descritos en [`LIQUIDITY_CHAIN_REACTION_AUDIT_E5_10_6.md`](../../../mapazapp/docs/LIQUIDITY_CHAIN_REACTION_AUDIT_E5_10_6.md). **`liquidity_chain_reaction_confirmed`** sigue la **heurística de ventana cerrada** auditada (coherente con `liquidity_sweep_reaction_score`), no el umbral del subscore ya repartido en cadena.
  Con build **E5.10.4+**, cuando el score de liquidez está habilitado, **`liquidity_event_score`** usa **`liquidity_chain_score`** si `liquidity_chain_detected=true`; si no hay cadena cualificada, el aporte se **limita** (cap 7 desde la calidad sweep bruto). Detalle y tokens: [`LIQUIDITY_CHAIN_REFINEMENT_E5_10_4.md`](../../../mapazapp/docs/LIQUIDITY_CHAIN_REFINEMENT_E5_10_4.md).  
  Con build **E5.10.2+** hasta **E5.10.3**, `liquidity_event_score` en el CSV reflejó el **total de calidad** (0–20) cuando el score de liquidez está habilitado; ver [`LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md`](../../../mapazapp/docs/LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md). **E5.10.2.1 — `liquidity_sweep_quality_reasons`:** lista `|`-separada de tokens **diagnósticos**; los subcondicionales son `liquidity_sweep_old`, `opposite_liquidity_sweep`, `liquidity_directional_partial`, `liquidity_sweep_no_reaction`, `liquidity_sweep_displacement_not_confirmed`, `liquidity_level_far_from_fvg`. **`liquidity_sweep_quality_weak`** aparece **como máximo una vez** y **solo** si el **grado final** es **Weak** (no se concatena junto a cada subfallo). **`liquidity_sweep_quality_ok`** se emite para grado **A** o **B**, y para **C** con total **≥ 12**. Los cinco subscores numéricos se **reparten proporcionalmente** para que su suma coincida con `liquidity_sweep_quality_score` tras los **topes** de contexto (reacción + desplazamiento + proximidad al FVG + dirección + recencia para permitir bandas A/B altas).  
  Cierre de fila (tras todas las extensiones anteriores): `session_bucket,trade_window_status,spread_status,news_mode`.  
  ...
  Los bundles **anteriores** sin columnas E5.8/E5.10 siguen siendo válidos (importador TS: columnas opcionales). Cuando existan columnas E5.8, `validateTestEaExportSample` exige coherencia con `has_entry_quality_score_logic` / `score_observation_only` / `score_gate_enabled`. Cuando `has_liquidity_sweep_v1_logic` es **true** en el summary, el validador exige las columnas E5.10 en la cabecera de trades y los contadores/resumen de liquidez documentados en [`LIQUIDITY_SWEEP_DETECTION_EXPORT_E5_10.md`](../../../mapazapp/docs/LIQUIDITY_SWEEP_DETECTION_EXPORT_E5_10.md). Cuando `has_liquidity_sweep_quality_v1_logic` es **true**, exige además las columnas E5.10.2 y los agregados de calidad en el summary (ver [`LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md`](../../../mapazapp/docs/LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md)). Cuando `has_liquidity_chain_v1_logic` es **true**, exige las columnas **E5.10.4** y los agregados documentados en [`LIQUIDITY_CHAIN_REFINEMENT_E5_10_4.md`](../../../mapazapp/docs/LIQUIDITY_CHAIN_REFINEMENT_E5_10_4.md). Cuando **`has_liquidity_chain_reaction_audit_v1_logic`** es **true**, exige las columnas **E5.10.6** y los contadores agregados en [`LIQUIDITY_CHAIN_REACTION_AUDIT_E5_10_6.md`](../../../mapazapp/docs/LIQUIDITY_CHAIN_REACTION_AUDIT_E5_10_6.md).
...
| `has_liquidity_sweep_quality_v1_logic` | boolean | **E5.10.2:** **true** cuando el EA exporta **Liquidity Sweep Quality V1** (subscores + columnas `liquidity_sweep_quality_*`); sigue siendo **observación** — sin compuerta dura. |
...
| Agregados calidad liquidez (E5.10.2, cuando `has_liquidity_sweep_quality_v1_logic`) | number | `average_liquidity_sweep_quality_score`, `liquidity_sweep_quality_a_count` … `liquidity_sweep_quality_none_count`, medias de subscores (`average_liquidity_sweep_recency_score`, … `average_liquidity_sweep_distance_score`), medias opcionales por outcome (`average_liquidity_sweep_quality_score_win`, … `…_expired_unfilled`). |
