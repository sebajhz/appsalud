...
| `show_runtime_status` | Ver estado del sistema | Show consolidated honest status | D4 model + **`GET /api/mapazapp/runtime/status`** (D5.1b) or launcher snapshot | API read-only snapshot exists; UI not wired | **No** (no dashboard panel yet) | Never show MT5/bridge “connected” without real checks | D6 panel |
