...
| `show_runtime_status` | Ver estado del sistema | Show consolidated honest status | D4 model + **future** read-only API or launcher snapshot | Model exists; no endpoint | **No** (design only) | Never show MT5/bridge “connected” without real checks | D5 / D6 |
