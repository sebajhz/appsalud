...
- **D14.7 (`@workspace/scripts`, wrapper real mínimo):** [`mapazapp-local-launcher-wrapper.ts`](../../../scripts/src/mapazapp-local-launcher-wrapper.ts) + tests + script **`mapazapp:launcher-wrapper`** — dry-run por defecto; **`--confirm-start`** para una corrida **run-once** vía **`runApiDashboardSupervisor`** (**D13.5**); salida humana/JSON; **sin** `.exe`/installer/packaging/layout writes; siguiente evidencia formal **D14.8**.
