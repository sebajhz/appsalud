...
- **D9.19–D10.4:** **`actionTransportReadiness.d9.test.ts`** — readiness transporte sin endpoint; **`MT5_DETECTION_GATE_AUDIT_D10.md`** — D10.0; **`mapazapp-mt5-config-model.ts`** — D10.1; **`MT5_OPEN_ACTION_DESIGN_D10.md`** — D10.2 (`open_mt5` diseño-only); **`mapazapp-mt5-runtime-status.ts`** — D10.3 (mapeo runtime conservador); **`Mt5ConfigStatusPanel`** — D10.4 UI draft read-only mock en Config. **Sin** lanzar MT5, **sin** watcher, **sin** POST de acciones.
