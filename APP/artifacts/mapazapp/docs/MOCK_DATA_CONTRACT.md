...
**Scanner simulation (checkpoint 12)** ← `runScannerSimulation` / `runCheckpoint12ScannerFixture` in `@workspace/mapazapp-core` + dashboard **`/scanner`** + optional **`GET /api/mapazapp/scanner/...`** — **synthetic candles or parsed Bridge CSV text only**; every payload is **`simulatedScanner: true`**, **`reviewOnly: true`**, **`executionEnabled: false`** — **not** a live scanner, **not** MT5 truth, **not** execution advice  
**Backtest evidence loop (checkpoint 15)** ← `evaluateBacktestEvidence` + fictional **`backtest-evidence-fixtures`** keyed to selected registry **`parameterSetId`** values + mock **`GET /api/mapazapp/backtest-evidence`** / per-set evidence & approval-proposal routes — **`advisoryOnly`**, **`registryMutationAllowed: false`**, **`canAutoApply: false`** — **not** Strategy Tester folder ingest, **not** registry mutation, **not** profitability proof  
**Forward / demo monitor (checkpoint 16)** ← **`evaluateForwardMonitorSnapshot`** + **`forward-monitor-fixtures`** + dashboard **`/forward-monitor`** + mock **`GET /api/mapazapp/forward-monitor/*`** — snapshot-only, **`reviewOnly`**, **`executionEnabled: false`**, **`mockOnly: true`** — **not** a live watcher, **not** WebSocket, **not** DB, **not** order execution; observational review discipline only  
**Assisted execution contract (checkpoints 17–18)** ← **`validateAssistedExecutionIntent`** + **`assisted-execution-invariants`** + **`assisted-execution-fixtures`** + dashboard **`/assisted-execution`** + mock **`GET /api/mapazapp/assisted-execution/*`** (including **`/safety`** and **`/invariants`**) — **contract and validation only**, **`contractOnly: true`**, **`mockOnly: true`**, **`executionEnabled: false`**, **`sendToMt5Enabled: false`**, **`canAutoExecute: false`**, **`registryMutationAllowed: false`**, **`manualReviewRequired: true`** — **not** broker submission, **not** MT5 command channel, **not** `POST` execution, **not** registry mutation; manual checklist / audit preview semantics only; **CP19+** required for any real execution decision
