/**
 * E5.22.2 — CLI: setup performance baseline audit (read-only; no MT5).
 */
...
import {
  ...
  type TestEaSetupPerformanceBaselineAuditResult,
...
export type SetupPerformanceBaselineAuditCliIo = {
...
const USAGE = `mapazapp-testea-setup-performance-baseline-audit (read-only, no MT5)

Usage:
  pnpm --filter @workspace/scripts mapazapp:testea-setup-performance-baseline-audit -- [options]
...
Options:
  ...
  --csv-output <path>       Write flattened audit summary CSV
...
function defaultIo(): SetupPerformanceBaselineAuditCliIo {
  return {
...
function writeAuditCsv(
  ...
  io: SetupPerformanceBaselineAuditCliIo,
): void {
  const rows = flattenSetupPerformanceBaselineAuditCsvRows(result);
  const lines = [
    "section,key,count,win_count,loss_count,ambiguous_count,expired_unfilled_count,total_r,avg_r,winrate,notes",
    ...rows.map(
      (r) =>
        `${csvEscape(r.section)},${csvEscape(r.key)},${r.count},${r.win_count},${r.loss_count},${r.ambiguous_count},${r.expired_unfilled_count},${r.total_r},${r.avg_r},${r.winrate},${csvEscape(r.notes)}`,
    ),
  ];
  io.writeFileUtf8(path, `${lines.join("\n")}\n`);
}

export function runSetupPerformanceBaselineAuditCli(
  argv: string[],
  io: SetupPerformanceBaselineAuditCliIo = defaultIo(),
): number {
  ...
  const results: TestEaSetupPerformanceBaselineAuditResult[] = [];
  for (const b of bundles) {
    ...
    const r = analyzeTestEaSetupPerformanceBaselineAuditFromTexts(loaded, { maxExamples });
    results.push(r);
    if (!r.ok) {
  ...
  if (jsonOut) {
    ...
    for (const r of okResults) {
      io.stdoutWrite(`\n=== ${r.bundle} (setup performance baseline — research) ===\n`);
      io.stdoutWrite(`ea_build=${r.ea_build} trades=${r.trade_count} total_r=${r.official_performance.total_r}\n`);
      io.stdoutWrite(
        `winrate=${r.official_performance.winrate.toFixed(4)} expectancy_r=${r.official_performance.expectancy_r.toFixed(4)}\n`,
      );
      io.stdoutWrite(`flags: ${r.flags.join(", ") || "(none)"}\n`);
      io.stdoutWrite(`hypotheses: ${r.hypotheses.length}\n`);
    }
  }

  if (csvOutput && okResults[0]) {
    writeAuditCsv(csvOutput, okResults[0], io);
...
if (isMain) {
  process.exit(runSetupPerformanceBaselineAuditCli(process.argv.slice(2)));
}
