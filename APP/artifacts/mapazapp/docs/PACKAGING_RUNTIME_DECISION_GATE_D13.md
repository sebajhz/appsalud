...
**Siguiente gate** (post-**D13.9**): [`LOCAL_LAUNCHER_PROTOTYPE_GATE_D14.md`](./LOCAL_LAUNCHER_PROTOTYPE_GATE_D14.md) (**D14.0** — local launcher prototype gate; secuencia recomendada **D14.1 → D14.2 → D14.3**; **sin** `.exe` ni código).
...
3. **D14.0 — Local launcher prototype gate**: documentado en [`LOCAL_LAUNCHER_PROTOTYPE_GATE_D14.md`](./LOCAL_LAUNCHER_PROTOTYPE_GATE_D14.md) — checklist **§7** + aprobación explícita; abre la secuencia **D14.1 (layout model) → D14.2 (packaging dry-run manifest) → D14.3 (wrapper prototype decision)** antes del primer `.exe` o bundle instalable; **sin** código en el propio **D14.0**.
