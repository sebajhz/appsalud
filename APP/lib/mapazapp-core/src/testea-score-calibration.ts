...
export const LIQUIDITY_TARGET_CALIBRATION_COLUMNS = ["liquidity_target_score"] as const;
...
/** E5.17 optional discipline score when CSV includes `discipline_score`. */
export const DISCIPLINE_CALIBRATION_COLUMNS = ["discipline_score"] as const;

export type DisciplineCalibrationColumn = (typeof DISCIPLINE_CALIBRATION_COLUMNS)[number];

/** E5.18 optional setup readiness score when CSV includes `setup_readiness_score`. */
export const SETUP_READINESS_CALIBRATION_COLUMNS = ["setup_readiness_score"] as const;

export type SetupReadinessCalibrationColumn = (typeof SETUP_READINESS_CALIBRATION_COLUMNS)[number];
 ...
 */
export const ENTRY_FILL_FEASIBILITY_CALIBRATION_COLUMNS = ["entry_fill_feasibility_score"] as const;
...
export interface TestEaScoreCalibrationBundleAnalysis {
  ...
  > | null;
  /**
   * Present when trades CSV includes E5.13.2 `entry_fill_feasibility_score`.
   * Post-candidate / outcome-explanation diagnostic — not for pre-trade gating.
   */
  ...
  > | null;
  /** Present when trades CSV includes E5.17 `discipline_score`. */
  discipline_component_stats: Partial<Record<DisciplineCalibrationColumn, TestEaScoreComponentStats>> | null;
  /** Present when trades CSV includes E5.18 `setup_readiness_score`. */
  setup_readiness_component_stats: Partial<
    Record<SetupReadinessCalibrationColumn, TestEaScoreComponentStats>
  > | null;
}
...
export interface TradeScoreAuxiliary {
  ...
  discipline: Partial<Record<DisciplineCalibrationColumn, number | null>> | null;
  setup_readiness: Partial<Record<SetupReadinessCalibrationColumn, number | null>> | null;
  missing_raw: string;
...
export function parseTradeScoreAuxiliaryByTradeId(tradesCsvText: string): {
  ...
  hasDisciplineScoreColumn: boolean;
  hasSetupReadinessScoreColumn: boolean;
  warnings: string[];
} {
  ...
  if (rows.length < 1) {
    ...
    return {
      ...
      hasDisciplineScoreColumn: false,
      hasSetupReadinessScoreColumn: false,
      warnings,
    };
  }
  ...
  const hasDisciplineScoreColumn = col.has("discipline_score");
  const hasSetupReadinessScoreColumn = col.has("setup_readiness_score");
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    let entry_fill_feasibility: Partial<Record<EntryFillFeasibilityCalibrationColumn, number | null>> | null = null;
    if (hasEntryFillFeasibilityScoreColumn) {
      entry_fill_feasibility = {};
      for (const c of ENTRY_FILL_FEASIBILITY_CALIBRATION_COLUMNS) {
        entry_fill_feasibility[c] = parseFiniteNumber(pick(cells, col, c));
      }
    }
    ...
    let liquidity_target: Partial<Record<LiquidityTargetCalibrationColumn, number | null>> | null = null;
    if (hasLiquidityTargetScoreColumn) {
      liquidity_target = {};
      for (const c of LIQUIDITY_TARGET_CALIBRATION_COLUMNS) {
        liquidity_target[c] = parseFiniteNumber(pick(cells, col, c));
      }
    }
    ...
    let discipline: Partial<Record<DisciplineCalibrationColumn, number | null>> | null = null;
    if (hasDisciplineScoreColumn) {
      discipline = {};
      for (const c of DISCIPLINE_CALIBRATION_COLUMNS) {
        discipline[c] = parseFiniteNumber(pick(cells, col, c));
      }
    }

    let setup_readiness: Partial<Record<SetupReadinessCalibrationColumn, number | null>> | null = null;
    if (hasSetupReadinessScoreColumn) {
      setup_readiness = {};
      for (const c of SETUP_READINESS_CALIBRATION_COLUMNS) {
        setup_readiness[c] = parseFiniteNumber(pick(cells, col, c));
      }
    }
  ...
  return {
    ...
    hasDisciplineScoreColumn,
    hasSetupReadinessScoreColumn,
    warnings,
  };
}
...
function buildEntryFillFeasibilityComponentStats(
  rows: { trade: BacktestTrade; aux: TradeScoreAuxiliary | undefined }[],
): Partial<Record<EntryFillFeasibilityCalibrationColumn, TestEaScoreComponentStats>> | null {
  const out: Partial<Record<EntryFillFeasibilityCalibrationColumn, TestEaScoreComponentStats>> = {};
  let any = false;
  for (const col of ENTRY_FILL_FEASIBILITY_CALIBRATION_COLUMNS) {
    const vals: number[] = [];
    const byOutcome: Partial<Record<TestEaScoreOutcomeGroup, { sum: number; count: number }>> = {};
    let allSum = 0;
    let allCnt = 0;
    for (const { trade, aux } of rows) {
      const v = aux?.entry_fill_feasibility?.[col];
      if (v == null || !Number.isFinite(v)) continue;
      any = true;
      vals.push(v);
      allSum += v;
...
function buildLiquidityTargetComponentStats(
  rows: { trade: BacktestTrade; aux: TradeScoreAuxiliary | undefined }[],
): Partial<Record<LiquidityTargetCalibrationColumn, TestEaScoreComponentStats>> | null {
  const out: Partial<Record<LiquidityTargetCalibrationColumn, TestEaScoreComponentStats>> = {};
  let any = false;
  for (const col of LIQUIDITY_TARGET_CALIBRATION_COLUMNS) {
    const vals: number[] = [];
    const byOutcome: Partial<Record<TestEaScoreOutcomeGroup, { sum: number; count: number }>> = {};
    let allSum = 0;
    let allCnt = 0;
    for (const { trade, aux } of rows) {
      const v = aux?.liquidity_target?.[col];
      if (v == null || !Number.isFinite(v)) continue;
      any = true;
      vals.push(v);
      allSum += v;
...
function buildDisciplineComponentStats(
  rows: { trade: BacktestTrade; aux: TradeScoreAuxiliary | undefined }[],
): Partial<Record<DisciplineCalibrationColumn, TestEaScoreComponentStats>> | null {
  const out: Partial<Record<DisciplineCalibrationColumn, TestEaScoreComponentStats>> = {};
  let any = false;
  for (const col of DISCIPLINE_CALIBRATION_COLUMNS) {
    const vals: number[] = [];
    const byOutcome: Partial<Record<TestEaScoreOutcomeGroup, { sum: number; count: number }>> = {};
    let allSum = 0;
    let allCnt = 0;
    for (const { trade, aux } of rows) {
      const v = aux?.discipline?.[col];
      if (v == null || !Number.isFinite(v)) continue;
      any = true;
      vals.push(v);
      allSum += v;
...
function buildSetupReadinessComponentStats(
  rows: { trade: BacktestTrade; aux: TradeScoreAuxiliary | undefined }[],
): Partial<Record<SetupReadinessCalibrationColumn, TestEaScoreComponentStats>> | null {
  const out: Partial<Record<SetupReadinessCalibrationColumn, TestEaScoreComponentStats>> = {};
  let any = false;
  for (const col of SETUP_READINESS_CALIBRATION_COLUMNS) {
    const vals: number[] = [];
    const byOutcome: Partial<Record<TestEaScoreOutcomeGroup, { sum: number; count: number }>> = {};
    let allSum = 0;
    let allCnt = 0;
    for (const { trade, aux } of rows) {
      const v = aux?.setup_readiness?.[col];
      if (v == null || !Number.isFinite(v)) continue;
      any = true;
      vals.push(v);
      allSum += v;
...
export function analyzeTestEaScoreCalibrationFromTexts(
  ...
  const analysis: TestEaScoreCalibrationBundleAnalysis = {
    ...
    execution_environment_component_stats: auxParse.hasExecutionEnvironmentScoreColumn
      ...
      : null,
    discipline_component_stats: auxParse.hasDisciplineScoreColumn
      ? buildDisciplineComponentStats(merged)
      : null,
    setup_readiness_component_stats: auxParse.hasSetupReadinessScoreColumn
      ? buildSetupReadinessComponentStats(merged)
      : null,
  };
