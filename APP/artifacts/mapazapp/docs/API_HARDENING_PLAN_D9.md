...
| **D9.16** | **Token / CSRF config model** — **`apiActionTokenConfig.ts`** + **`apiActionTokenConfig.d9.test.ts`** — **no** **`app.ts`** wiring | **Implemented:** `ApiActionTokenConfig`, env bag parsing, `validateApiActionTokenConfig` — **no** secret values, **no** verification runtime |
| **D9.17** | **Token verification middleware skeleton** — **`actionTokenMiddleware.ts`** — **not** mounted in **`app.ts`** — **no** Mapazapp action **`POST`** | **Implemented:** `createActionTokenMiddleware` + temporary Express apps in tests only — production **`app.ts`** unchanged |
| **D9.18** | **Token tests + redaction integration** — **`actionTokenMiddleware.d9.test.ts`**, **`logRedaction.d9.test.ts`**, readiness §J — **no** action **`POST`** | **Implemented:** missing/invalid/query/cookie-negative cases; **`sanitizeLogString`** action-token fragments; static scan — **no** product route wiring |
...
- **D9.15** — [`API_TOKEN_CSRF_DESIGN_D9.md`](./API_TOKEN_CSRF_DESIGN_D9.md) — token / CSRF posture (**documentation only**).
- **D9.16–D9.18** — **`apiActionTokenConfig.ts`**, **`actionTokenMiddleware.ts`** (unwired), vitest coverage + **`logRedaction`** action-token scrubbing — **no** Mapazapp action **`POST`**, **no** real token issuance; transport wiring remains **D9.19**+.
