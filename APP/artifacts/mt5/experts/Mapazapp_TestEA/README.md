# Mapazapp_TestEA — Official Strategy Tester EA (E3.6–E5.10.2.1)
...
| **Liquidity sweep (E5.10–E5.10.2.1)** | **Observation-only** PDH/PDL + local M15 swing sweep **context**; **E5.10.2** adds **Liquidity Sweep Quality V1** subscores + `liquidity_sweep_quality_*` columns; **E5.10.2.1** refina razones (sin `quality_weak` duplicado) y topes A/B; `liquidity_event_score` tracks the **quality total** (max 20) when scoring is enabled; **`has_liquidity_sweep_v1_logic`** + **`has_liquidity_sweep_quality_v1_logic`**; **no** hard gate — see [`LIQUIDITY_SWEEP_DETECTION_EXPORT_E5_10.md`](../../../mapazapp/docs/LIQUIDITY_SWEEP_DETECTION_EXPORT_E5_10.md) and [`LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md`](../../../mapazapp/docs/LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md). |
...
2. Compile in **MetaEditor** (F7) as **`MZP_TestEA_E5_10_2_1`**.
...
- **Compile** build **`MZP_TestEA_E5_10_2_1`** (see `TESTEA_BUILD` in `Mapazapp_TestEA.mq5`).
