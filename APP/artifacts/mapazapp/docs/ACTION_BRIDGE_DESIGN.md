...
**Related:** [`DASHBOARD_RUNTIME_ACTIONS_DESIGN.md`](./DASHBOARD_RUNTIME_ACTIONS_DESIGN.md) (D4.1 — action IDs and safety rules), [`LAUNCHER_CONFIG_AND_STATUS_DESIGN.md`](./LAUNCHER_CONFIG_AND_STATUS_DESIGN.md) (D2 — config and runtime semantics), [`RUNTIME_AND_LAUNCHER_STRATEGY.md`](./RUNTIME_AND_LAUNCHER_STRATEGY.md), [`LAUNCHER_PROTOTYPE_DESIGN_D8.md`](./LAUNCHER_PROTOTYPE_DESIGN_D8.md) (**D8.1** — launcher prototype + launcher-side bridge design, **documentation only**), [`LOCAL_ACTION_BRIDGE_THREAT_MODEL_D9.md`](./LOCAL_ACTION_BRIDGE_THREAT_MODEL_D9.md) (**D9.1** — formal localhost / action-bridge threat model and mandatory mitigations — **must precede any `POST` / action endpoint**), [`LOCAL_ACTION_TRANSPORT_CONTRACT_D9.md`](./LOCAL_ACTION_TRANSPORT_CONTRACT_D9.md) (**D9.6** — formal transport contract: HTTP loopback + IPC minimums, caller remapping — **docs only**; **`POST` still not implemented**), [`LOCAL_ACTION_TRANSPORT_TEST_PLAN_D9.md`](./LOCAL_ACTION_TRANSPORT_TEST_PLAN_D9.md) (**D9.7** — transport safety test plan before implementation — **docs only**).
...
- **Before** adding buttons or `POST` endpoints, the repo needs a **single written bridge contract**: who may spawn processes, how the browser participates, and how results return safely — the formal **threat model** in [**D9.1** `./LOCAL_ACTION_BRIDGE_THREAT_MODEL_D9.md`](./LOCAL_ACTION_BRIDGE_THREAT_MODEL_D9.md) and the **transport contract** in [**D9.6** `./LOCAL_ACTION_TRANSPORT_CONTRACT_D9.md`](./LOCAL_ACTION_TRANSPORT_CONTRACT_D9.md) must be satisfied by any future HTTP/IPC transport design; **`POST` action routes remain unimplemented** until those obligations are met in code review.
...
| **D8.3** | Launcher **preflight-only bridge** — `APP/scripts/src/mapazapp-launcher-preflight-bridge.ts` (**implemented**); returns safe **`MapazappActionResult`** for **`validate_environment`**; **no** HTTP `POST`, **no** dashboard wiring |
