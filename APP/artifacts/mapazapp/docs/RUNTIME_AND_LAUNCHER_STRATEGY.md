...
- **D14.1 (solo documentación):** [`LOCAL_RUNTIME_FOLDER_LAYOUT_MODEL_D14.md`](./LOCAL_RUNTIME_FOLDER_LAYOUT_MODEL_D14.md) — modelo de **layout de carpetas de runtime local** (`launcher/`, `api-server/`, `dashboard/`, `config/`, `logs/`, `evidence/`, `runtime/`, `backups/`, `support/`), política **`.gitignore`**, paths, validaciones futuras y enlace a **D14.2** manifest; **sin** crear carpetas en disco, **sin** `fs` de layout, **sin** empaquetado ni procesos.
- **D14.2 (solo documentación):** [`PACKAGING_DRY_RUN_MANIFEST_D14.md`](./PACKAGING_DRY_RUN_MANIFEST_D14.md) — contrato de **manifest dry-run** (metadata, artefactos fuente, mapeo a layout **D14.1**, requeridos/opcionales/excluidos, runtime-generated, políticas, dashboard A–E, validaciones y flags de seguridad); **sin** copiar archivos, **sin** JSON ejecutable fuera de docs, **sin** `.exe`.
...
- **D14.7 (`@workspace/scripts`, wrapper real mínimo):** [`mapazapp-local-launcher-wrapper.ts`](../../../scripts/src/mapazapp-local-launcher-wrapper.ts) + tests + script **`mapazapp:launcher-wrapper`** — dry-run por defecto; **`--confirm-start`** para una corrida **run-once** vía **`runApiDashboardSupervisor`** (**D13.5**); salida humana/JSON; **sin** `.exe`/installer/packaging/layout writes; siguiente evidencia formal **D14.8** (pausada mientras la fase motor **E** avance).

**Pausa estratégica:** *Runtime/launcher work is paused until the engine/setup proof phase reaches evidence* — ver [`ENGINE_SETUP_PROOF_MASTER_PLAN_E1.md`](./ENGINE_SETUP_PROOF_MASTER_PLAN_E1.md) (**E1**). No implica borrar supervisores/wrapper; sí implica **no** priorizar nueva expansión de packaging, `.exe` ni MT5 live hasta **E4/E5** mínimos del plan **E**.
