...
## Siguiente paso operativo — E5.10.7

1. Recompilar **`MZP_TestEA_E5_10_6`** en MetaEditor.
2. Ejecutar smoke Strategy Tester (misma campaña que E5.10.5 si aplica).
3. Validar CLI `mapazapp:testea-export-validate` + revisar distribución de:

   - `liquidity_chain_reaction_checked_count`
   - contadores `*_fail_*`
   - `liquidity_chain_reaction_confirmed_count`

4. Confirmar que la cadena sigue **observation-only** y que los nuevos campos **explican** fallos sin inflar artificialmente A/B.
