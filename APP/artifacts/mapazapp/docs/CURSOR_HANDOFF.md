...
- **D12.0–D12.1:** primer run **API-only** supervisado ejecutado **OK** sobre commit **`b267b70`**; evidencia documental **`API_ONLY_RUN_EVIDENCE_D12.md`** (D12.1). **D12.2:** plan documental **`API_DASHBOARD_SUPERVISED_RUN_PLAN_D12.md`** (API+dashboard futuro, **sin** ejecución en D12.2); primera ejecución API+dashboard ⇒ **nueva** aprobación (p. ej. **D12.3** en ese doc).
