...
- **E5.10.2 cerrado (repo):** **Liquidity Sweep Quality V1** — builds **`MZP_TestEA_E5_10_2`** / **`MZP_TestEA_E5_10_2_1`**; columnas `liquidity_sweep_quality_*`; `has_liquidity_sweep_quality_v1_logic`; `liquidity_event_score` alineado al total de calidad (0–20) cuando el score de liquidez está habilitado; **E5.10.2.1** corrige semántica de razones (`quality_weak` solo en grado Weak; `quality_ok` en A/B y C≥12) y topes de contexto para bandas altas — analizador E5.9 extendido si el CSV trae columnas — [`LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md`](./LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md).
