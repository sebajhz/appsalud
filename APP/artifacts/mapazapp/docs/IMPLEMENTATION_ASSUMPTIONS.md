...
- **Known polish (TODO, not implemented):** `bridge_status.json` **`errors_count`** currently counts buffered diagnostic rows; a lone **INFO** row (e.g. startup) can set **`errors_count`** to `1`. Future work may count only **WARNING**/**ERROR**, add a separate **`diagnostics_count`**, or clarify/rename the field — **contract columns unchanged** until an explicit spec change.
