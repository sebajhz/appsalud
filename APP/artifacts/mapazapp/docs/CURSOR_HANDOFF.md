...
- **Siguiente paso canónico:** **E5.10.7** — smoke Strategy Tester post–**E5.10.6** (recompilar `MZP_TestEA_E5_10_6`, mismo smoke **XAUUSD M15/D1**, validar bundle por CLI, revisar `liquidity_chain_reaction_confirmed_count`, razones de fallo y score/outcome solo como observación). **Roadmap intermedio humanización trader profesional (E5.11–E5.19, docs):** [`PROFESSIONAL_TRADER_HUMANIZATION_ROADMAP_E5_11.md`](./PROFESSIONAL_TRADER_HUMANIZATION_ROADMAP_E5_11.md) — HTF estructura, MSS/CHoCH, premium/discount, IFVG/BISI/SIBI, targets, sesión/noticias, riesgo/frecuencia, contrato BridgeEA/dashboard read-only, compuerta demo forward; invariantes: definiciones técnicas, exports, reason codes, counters; observación-first; sin `OrderSend`/`CTrade`/`PositionOpen`/`WebRequest` en esta línea hasta compuertas explícitas. **Nota:** el track opcional **`tester_orders`** (E5.1) queda **aparte**.
