...
- **D13.7** [`PACKAGING_RUNTIME_DECISION_GATE_D13.md`](./PACKAGING_RUNTIME_DECISION_GATE_D13.md) actualiza esa decisión **después** de supervisores **D13.2–D13.6**: **packaging / runtime** y secuencia **D13.8 → D13.9 → D14.0** (**solo** documentación en **D13.7**).
...
| Supervisor **API + dashboard** (D13.5) | **Implementado** en `@workspace/scripts`; evidencia formal del primer run en **D13.6** ([`API_DASHBOARD_SUPERVISOR_RUN_EVIDENCE_D13.md`](./API_DASHBOARD_SUPERVISOR_RUN_EVIDENCE_D13.md)). |
...
3. **D13.3** — evidencia run API-only cerrada: [`API_ONLY_SUPERVISOR_RUN_EVIDENCE_D13.md`](./API_ONLY_SUPERVISOR_RUN_EVIDENCE_D13.md).
...
- [x] Recomendación explícita y secuencia **D13.1 → D13.2 → D13.3 → D13.4 → D13.5–D13.7** registrada (refinada en **D13.4** §12 y **D13.1** §11 histórico API-only).
