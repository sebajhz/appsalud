# Mapazapp_TestEA — Official Strategy Tester EA (E3.6–E5.3)
...
| **Trading** | **No** `OrderSend`, **no** `CTrade`, **no** broker orders. **E5.3:** optional **virtual trade simulation** writes **CSV rows** and `virtual_trade_*` events; `has_real_trading_orders` remains **false**; `trade_count` matches exported rows (0 if disabled or no closes). |
...
**Next:** **E5.4** — first Strategy Tester smoke with **virtual outcomes** when the operator runs MT5 (not automated from this repo).
