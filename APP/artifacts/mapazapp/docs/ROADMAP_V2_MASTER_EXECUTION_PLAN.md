...
- **E3.4.2 — Merge lógica BacktestEA → Mapazapp_TestEA:** migrar esqueleto tester-only, Daily Bias V1, events/summary y tests al EA oficial de Strategy Tester; actualizar contratos/README; decidir deprecación de carpeta `Mapazapp_BacktestEA/`. **Compuerta antes de E3.5.**
...
- **Secuencia E (actualizada):** **E3.1** alineación tester → **E3.2** contrato Setup V1 EA → **E3.3** esqueleto EA + guard tester-only → **E3.4** Daily Bias en artefacto `Mapazapp_BacktestEA` → **E3.4.1** reconciliación roles / guía viva → **E3.4.2** merge en **`Mapazapp_TestEA`** → **E3.5** detección Setup V1 (IFVG) en **`Mapazapp_TestEA`** → **E3.6** esquema export evidencia → **E4** primer smoke tester → **E5** campaña XAUUSD tester → **E6** import evidencia MT5 en Mapazapp → **E7** diseño dashboard resultados → **E8** compuerta decisión setup.
