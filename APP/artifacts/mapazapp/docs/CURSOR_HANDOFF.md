...
- **E5.1 cerrado (decisión docs-only):** modo outcome — [`TESTEA_TRADE_OUTCOME_MODE_DECISION_E5_1.md`](./TESTEA_TRADE_OUTCOME_MODE_DECISION_E5_1.md): **virtual dentro de `Mapazapp_TestEA` + Strategy Tester primero**; **`tester_orders`** como track **opcional separado** (no confundir con **E5.6** actual = ambiguous/diagnóstico — ver [`PROFESSIONAL_SETUP_ENTRY_AUDIT_E5_5_2.md`](./PROFESSIONAL_SETUP_ENTRY_AUDIT_E5_5_2.md) §15).
...
- **Siguiente paso canónico:** **E5.6** — sensibilidad y diagnóstico `ambiguous` + exports de diagnóstico (plan §10 del audit); seguir **E5.7**–**E5.13** en audit/roadmap. **Nota:** el track opcional **`tester_orders`** (mención histórica en E5.1) queda **aparte** de esta numeración E5.6+ — ver **E5.5.2** §15.
