...
D11.3 fija **disciplina y orden** para un primer run local **posterior**; los modelos **D11.4–D11.5** describen ciclo de vida e instancia/puertos **solo en tipos**, sin runtime real. **D11.6** documenta start/stop seguro futuro del launcher; **D11.7** elige API-only como primer prototipo supervisado preferido; **D11.8** es la **compuerta humana** antes del primer run real; **D11.9** concreta el **plan API-only** (comandos precondiciones y evidencia). La ejecución material queda **fuera** de D11.3, **fuera** de D11.8–D11.9 y solo tras checklist firmado + checkpoint de ejecución (**p. ej. D12.0**).
