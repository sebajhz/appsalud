...
**Assisted execution contract (checkpoint 17)** ← **`validateAssistedExecutionIntent`** + **`assisted-execution-fixtures`** + dashboard **`/assisted-execution`** + mock **`GET /api/mapazapp/assisted-execution/*`** — **contract and validation only**, **`contractOnly: true`**, **`executionEnabled: false`**, **`sendToMt5Enabled: false`**, **`canAutoExecute: false`** — **not** broker submission, **not** MT5 command channel, **not** `POST` execution, **not** registry mutation; manual checklist / audit preview semantics only
