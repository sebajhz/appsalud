...
- **E5.10.2 cerrado (repo):** **Liquidity Sweep Quality V1** — build **`MZP_TestEA_E5_10_2`**; columnas `liquidity_sweep_quality_*`; `has_liquidity_sweep_quality_v1_logic`; `liquidity_event_score` alineado al total de calidad (0–20) cuando el score de liquidez está habilitado; analizador E5.9 extendido si el CSV trae columnas — [`LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md`](./LIQUIDITY_SWEEP_QUALITY_REFINEMENT_E5_10_2.md).
- **Siguiente paso canónico:** **E5.10.3** — smoke Strategy Tester + comparación de **liquidity quality** por outcome (operador). **Nota:** el track opcional **`tester_orders`** (E5.1) queda **aparte**.
